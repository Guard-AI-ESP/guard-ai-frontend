...
<div class="flex flex-col gap-3">
	...
	<div class="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-soft">
	...
	<div class="flex items-center gap-2 flex-wrap">
		...
		<div class="relative">
			...
			{#if sortOpen}
				<div class="absolute left-0 sm:right-0 sm:left-auto top-full mt-1 bg-white rounded-xl shadow-lg border border-slate-100 py-1 z-30 min-w-[180px]">
...
<div class="sm:hidden flex flex-col gap-3">
	{#if filtered.length === 0}
		<p class="py-12 text-center text-slate-400 text-sm">Aucun message trouvé</p>
	{:else}
		{#each filtered as msg}
			{@const days = daysLeft(msg.expiresAt)}
			<div class="bg-white rounded-2xl shadow-soft border border-slate-100 p-4 flex items-center gap-3 {!msg.listened ? 'border-l-2 border-l-primary' : ''}">
				<button
					aria-label="Écouter"
					onclick={() => playing = playing === msg.id ? null : msg.id}
				...
				>
					<span class="material-icons text-[20px]">{playing === msg.id ? 'pause' : 'play_arrow'}</span>
				</button>
				<div class="flex-1 min-w-0">
					<div class="flex items-center gap-1.5">
						...
						{/if}
						<p class="text-sm font-semibold text-slate-800 truncate">{msg.sender}</p>
					</div>
					<p class="text-xs text-slate-400 mt-0.5">{msg.camera} • {formatDate(msg.dateTs)} • {formatDuration(msg.durationSec)}</p>
				</div>
